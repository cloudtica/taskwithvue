<template>
    ...
    <div class="row full-width">
      ...
      <div class="col-xs-12 xol-sm-12 col-md-12 col-lg-12 adminheadingpadding">
        <h5 class="text-white text-center margin-0 "> All Users Records </h5>
      </div>
    ...
    <q-markup-table dark class="bg-indigo-8">
      ...
      <tbody>
        <tr v-for="admin in admins" v-bind:key="admin.id">
          <td class="text-left">{{admin.id}}</td>
          <td class="text-left">{{admin.name}}</td>
          <td class="text-left">{{admin.email}}</td>
          <td class="text-left">{{admin.country}}</td>
          <td class="text-left"><q-btn icon="fas fa-trash"  @click="deleteAdmin(admin.id)" color="warning" style="margin:0px 10px;"></q-btn>
            <q-btn icon="fas fa-edit"  @click="prepareUpdateForm(admin)" color="orange" style="margin:0px 10px;"></q-btn>
            <q-btn label="Change Password"  @click="preparePasswordForm(admin)" color="orange" style="margin:0px 10px;"></q-btn>
        </td>
        </tr>
      </tbody>
    </q-markup-table>
  ...
  <q-dialog class="full-width" v-model="showUpdateForm" persistent>
    <q-card>
      <q-card-section class="row" style="justify-content: center; display: flex; background-color:#1976d2;">
        <h6 class="text-white margin-0">Edit Admin Details</h6>
      </q-card-section>
      ...
      />
        ...
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
            <q-select style="width:400px;  margin:0px auto;"  square v-model="dataModel.country" :options="options" emit-value map-options label="Select Country" />
        </div>
      </q-card-section>
      ...
      <q-card-actions align="right">
        ...
        <q-btn
          ...
          color="primary"
          v-close-popup
          @click="updateAdmin(dataModel.id,dataModel.name,dataModel.email,dataModel.country)"
        />
      </q-card-actions>
    </q-card>
  </q-dialog>

  <q-dialog class="full-width" v-model="showPasswordForm" persistent>
    <q-card>
      <q-card-section class="row" style="justify-content: center; display: flex; background-color:#1976d2;">
        <h6 class="text-white margin-0">Edit Admin Password</h6>
      </q-card-section>
      ...
      <q-card-actions align="right">
        ...
        <q-btn
          ...
          color="primary"
          v-close-popup
          @click="updatePassword(dataModel.id,password,c_password)"
        />
      </q-card-actions>
    </q-card>
  </q-dialog>
      ...
      async loadData() {
        ...
        let response: any;
            try {
                ...
                response = await axios.get(api.get_admins
                , {
                    headers: {
                        Authorization: token,
                    },
        ...
        async deleteAdmin(adminId: number) {
                    ...
                    try {
                        let token: string = getAuthorizationToken();
                        response = await axios.post(api.delete_admin, formData
                        , {
                            headers: {
                                Authorization: token,
        ...
        prepareUpdateForm(adminsObj: any) {
          this.dataModel = JSON.parse(JSON.stringify(adminsObj));
          this.showUpdateForm = true;
        
                },
        preparePasswordForm(adminsObj: any) {
          this.dataModel = JSON.parse(JSON.stringify(adminsObj));  
          this.showPasswordForm=true;
                },
        
        
        async updateAdmin(dataId: number, name: string, email: string, formCountry: string) {
                    ...
                    try {
                        let token: string = getAuthorizationToken();
                        response = await axios.post(api.update_admin, formData
                        , {
                            headers: {
                                Authorization: token,
                            },
                ...
                if (err.response && err.response.status === 401) {
                  this.logout();
                    ...
                    async updatePassword(dataId: number, password: string, c_password: string) {
