<template>
    ...
    <div class="row full-width">
      ...
      <div class="col-xs-12 xol-sm-12 col-md-12 col-lg-12 adminheadingpadding">
        <h5 class="text-white text-center margin-0 "> Special Thanks Members Records </h5>
      </div>
      ...
      async loadData() {
        ...
        let response: any;
            try {
                ...
                response = await axios.get(api.get_thanks
                , {
                    headers: {
                        Authorization: token,
                    },
        ...
        async deleteTeamMember(teamsId: number) {
                    ...
                    try {
                        let token: string = getAuthorizationToken();
                        response = await axios.post(api.delete_thanks, formData
                        , {
                            headers: {
                                Authorization: token,
                ...
                if (err.response && err.response.status === 401) {
                  this.logout();
                    ...
                    if (response && response.data.error === true && response.data.message_key === 'validation_failed') {
                        ...
                        this.$router.push('/thanks/records');
                    }
        ...
        async updateMember(dataId: number, name: string, post: string, sorting: string, image: string) {
                    ...
                    try {
                        let token: string = getAuthorizationToken();
                        response = await axios.post(api.update_thanks, formData
                        , {
                            headers: {
                                Authorization: token,
                            },
