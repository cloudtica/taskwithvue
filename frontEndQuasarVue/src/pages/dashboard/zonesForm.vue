<template>
    ...
    <div class="row full-width">
      ...
      <div class="col-xs-12 xol-sm-12 col-md-12 col-lg-12 adminheadingpadding">
        <h5 class="text-white text-center margin-0 "> Fossil Free Zones Form Records </h5>
      </div>
    ...
    <q-markup-table dark class="bg-indigo-8">
      ...
      <tbody>
        <tr v-for="from in forms" v-bind:key="from.id">
          ...
          <td class="text-left">{{from.place}}</td>
          <td class="text-left">{{from.site}}</td>
          <td class="text-left">{{from.country}}</td>
      ...
      async loadData() {
        ...
        let response: any;
            try {
              ...
              // console.info(this.getDataFromLocalStorageByKey('country'));
              // if(superAdmin == 1 || superAdmin == '1') {
               
              //   }

                let token: string = getAuthorizationToken();
                response = await axios.get(api.get_zonesform_data
                , {
                    headers: {
                        Authorization: token,
                    },
            ...
            if (response && response.data.error === false) {
                if(response.data.data.length > 0) {
                  ...
                  this.users = response.data.data_users;
        ...
        async deleteRecord(formId: number) {
                    ...
                    try {
                        let token: string = getAuthorizationToken();
                        response = await axios.post(api.delete_zform, formData
                        , {
                            headers: {
                                Authorization: token,
